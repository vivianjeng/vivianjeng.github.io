...
> Comparison between the usage of multiple variables and an array as input formats. Using array as input type causes a gas consumption increase by 18508 (16.8%) in deployment and 190 (0.86%) in execution, respectively.
...
Because Solidity automatically converts a function into a method ID, which is the first (left, high-order in big-endian) four bytes of theKeccak-256 (SHA-3) hash of the signature of the function.
During compiling time, compiler builds a binary search tree according to the method ID, and sort these functions from the smallest method ID to the biggest one.
Once a function is called, an EVM finds the function from the root of a binary search tree.
However, Solidity compiler does not optimize this binary search tree.
If we know which function is going to be called many times in advance, we can rename this function to have a better method ID to fit in a better place of the binary search tree.
